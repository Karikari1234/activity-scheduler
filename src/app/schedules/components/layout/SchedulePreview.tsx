...
interface SchedulePreviewProps {
  ...
  selectedDate?: string;
  schedules?: any[]; // Using the Schedule type
}
        ...
        <div className="preview-content">
          <div className="bg-surface rounded-md overflow-hidden shadow-sm border border-divider">
            <div className="bg-primary-light p-md flex justify-between items-center">
              <span className="text-lg font-medium text-primary">Meeting Schedule</span>
              <span className="text-primary-dark font-medium">{formatDate(selectedDate)}</span>
            </div>
            ...
            <div className="p-md">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="p-sm border-b-2 border-divider text-left text-sm font-medium text-text-secondary uppercase tracking-wide">Time</th>
                    <th className="p-sm border-b-2 border-divider text-left text-sm font-medium text-text-secondary uppercase tracking-wide">Place</th>
                    <th className="p-sm border-b-2 border-divider text-left text-sm font-medium text-text-secondary uppercase tracking-wide">Activity</th>
                    <th className="p-sm border-b-2 border-divider text-left text-sm font-medium text-text-secondary uppercase tracking-wide">Comment</th>
                  </tr>
                </thead>
                <tbody>
                  {schedules.map((schedule, index) => (
                    <tr key={schedule.id} className={index % 2 === 0 ? "bg-background" : ""}>
          ...
          <div className="mt-md flex justify-end">
            <Button 
              ...
              onClick={() => {
                ...
                alert('PDF download functionality will be implemented separately');
              }}
            >
